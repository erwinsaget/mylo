  ...
  describe('Validation', function () {
    it('Does not allow user to register without a name', function (done) {
      chai.request(app).post('/users').send({
        ...
        expect(response.body.message).to.eql('Field name does not exist. (required)');
        done();
      });
    });
    ...
    it('Does not allow user to register without an email', function (done) {
      chai.request(app).post('/users').send({
        ...
        expect(err).to.be.null;
        expect(response).to.have.status(400);
        expect(response.body.message).to.eql('Field email does not exist. (required)');
        done();
      });
    });
    ...
    it('Does not allow user to register without a password', function (done) {
      chai.request(app).post('/users').send({
        ...
        expect(err).to.be.null;
        expect(response).to.have.status(400);
        expect(response.body.message).to.eql('Field password does not exist. (required)');
        done();
      });
    });
  });
